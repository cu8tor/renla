...
function SoonPage({ item }) {
  const copy = {
    ...
    performance: "KPIs, quarterly reviews and manager feedback are planned for a later phase once the core is proven.",
  };
  return (
    <div className="cp-fade" style={{ maxWidth: 560 }}>
      <PageHead title={item.label} sub="Coming soon" />
      <Card pad={32} style={{ textAlign: "center" }}>
        ...
        <h3 style={{ fontFamily: "var(--font-display)", fontSize: 19, fontWeight: 700, margin: "0 0 8px" }}>{item.label}</h3>
        <p style={{ color: "var(--muted)", fontSize: 14, lineHeight: 1.6, maxWidth: 400, margin: "0 auto" }}>{copy[item.key]}</p>
        <div style={{ marginTop: 18 }}><Badge tone="brand">Coming soon</Badge></div>
      </Card>
    </div>
  );
}

...
This is the V1 milestone from the commercialisation roadmap: pull the highest-value business logic out of `App.jsx` into standalone, tested modules — with **zero behaviour change** — and rebrand the app from "Cura People" to "Renla." Business logic behaviour is unchanged throughout except for the one deliberate fix noted below; what moved is where the code lives and what it's called.
...
- **`src/App.jsx`** — same file, same behaviour (except the Rota fix below), now importing from all three feature modules instead of defining this logic inline. Also renamed throughout: loading screen, sidebar/brand text, backup filename, payslip label, and explanatory copy now say "Renla" instead of "Cura People" (10 occurrences). Deliberately **not** changed: the internal `DEVICE_KEY` localStorage key (`"cura:device:v1"`) and the `cp-` CSS class prefixes — both are internal-only, and changing the device key specifically would silently un-register every device already approved in the live Cura People instance.
